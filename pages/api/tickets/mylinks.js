...
export default async function handler(req, res) {
  try {
    ...
    const tickets = await TicketModel.find({ seller: decoded._id })
      .populate({
        path: "ticketType",
    ...
    for (const ticket of tickets) {
      if (ticket.group in links) {
        links[ticket.group].push(ticket);
      } else {
        links[ticket.group] = [ticket];
      }
    }

    return res.status(200).json({
      status: true,
      message: "List of all Links",
      data: links,
    });
  } catch (error) {
    console.log(error);
